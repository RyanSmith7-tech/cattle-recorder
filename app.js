const HerdIQ=(()=>{const KEY="HERDIQ_V60", LEGACY_KEYS=["HERDIQ_V52","HERDIQ_V51","HERDIQ_V50"], DB_NAME="HERDIQ_OFFLINE", DB_STORE="snapshots";
...
function exportBackup(){save();let payload={format:"Central Vets HerdIQ Backup",version:60,exportedAt:new Date().toISOString(),state:S};downloadBlob(new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),`HerdIQ_FULL_BACKUP_${new Date().toISOString().slice(0,10)}.json`)}
...
return{show,quickStart,repeatLast,applyPreset,saveStation,loadStationDefaults,addDraftRule,removeDraft,clearDraftRules,draftPreset,renderCalvingGuide,startSession,setYard,finishSession,addCalendarEvent,renderCalendar,loadStrategyDefaults,calculateEconomics,saveScenario,calculateBullPower,saveBullPower,runEvidenceEngine,saveEvidenceRun,renderHistory,openSession,renderReport,exportCSV,downloadPDF,sharePDF,emailDetails,editRecord,backField,exportBackup,importBackup,requestPersistentStorage,renderStorageStatus,clearAll}
})();