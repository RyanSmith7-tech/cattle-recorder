const HerdIQ=(()=>{const KEY="HERDIQ_V51";
...
function save(){localStorage.setItem(KEY,JSON.stringify(S))}function load(){let r=localStorage.getItem(KEY)||localStorage.getItem("HERDIQ_V50");if(r)S=JSON.parse(r);S.stations??=[];S.sessions??=[];S.calendarEvents??=[];S.economicScenarios??=[];S.bullPowerScenarios??=[];S.evidenceRuns??=[];S.records??=[];S.draftRules??=[];S.yard??={};S.animal??={}}
...
function draftPreset(type){S.draftRules=type==="heifers"?[{id:uid(),name:"Empty heifers",classes:["H"],pregs:["E"]},{id:uid(),name:"P1-P3 heifers",classes:["H"],pregs:["P1","P2","P3"]},{id:uid(),name:"P4-P6 heifers",classes:["H"],pregs:["P4","P5","P6"]},{id:uid(),name:"P7-P9 heifers",classes:["H"],pregs:["P7","P8","P9"]}]:[{id:uid(),name:"Empty cows",classes:["C","A"],pregs:["E"]},{id:uid(),name:"P1-P3 cows",classes:["C","A"],pregs:["P1","P2","P3"]},{id:uid(),name:"P4-P6 cows",classes:["C","A"],pregs:["P4","P5","P6"]},{id:uid(),name:"P7-P9 cows",classes:["C","A"],pregs:["P7","P8","P9"]},{id:uid(),name:"Speyed",classes:["S"],pregs:[]}];save();renderDraftRules()}
...
function activeFields(){let s=S.active;if(!s)return[];let arr=[];if(s.fields.class&&!s.defaultClass)arr.push(F.classCode);if(s.fields.age)arr.push(F.age);if(s.fields.bcs)arr.push(F.bcs);if(s.fields.preg)arr.push(F.preg);let cls=S.animal.classCode||s.defaultClass;if(s.fields.weight&&(s.weightMode==="all"||(cls==="H"&&s.weightMode==="heifers")))arr.push(F.weight);arr.push(F.wetDry);return arr}
const F={
 classCode:{k:"classCode",label:"CLASS",prompt:"ENTER CLASS",hint:"H heifer / C cow / S speyed / A aged",auto:true,val:v=>["H","C","S","A"].includes(v)?v:null,disp:v=>classNames[v]},
 age:{k:"age",label:"AGE",prompt:"ENTER AGE",hint:"0-9",auto:true,val:v=>/^[0-9]$/.test(v)?+v:null,disp:v=>v},
 bcs:{k:"bcs",label:"BCS",prompt:"ENTER BCS",hint:"10, 15, 20 … 50",auto:false,val:v=>{v=String(v).replace(".","");if(/^[1-5]$/.test(v))return +v;if(/^(10|15|20|25|30|35|40|45|50)$/.test(v))return +v/10;return null},disp:v=>Number(v).toFixed(1)},
 preg:{k:"preg",label:"PREG",prompt:"ENTER PREG",hint:"E or 1-9",auto:true,val:v=>v==="E"?"E":/^[1-9]$/.test(v)?"P"+v:null,disp:v=>v},
 weight:{k:"weight",label:"WEIGHT",prompt:"ENTER WEIGHT",hint:"kg + Enter",auto:false,val:v=>+v>0?+v:null,disp:v=>v+"kg"},
 wetDry:{k:"wetDry",label:"WET / DRY",prompt:"ENTER WET OR DRY",hint:"W or D",auto:true,val:v=>["W","D"].includes(v)?v:null,disp:v=>wetDryNames[v]}
};
...
return{show,quickStart,repeatLast,applyPreset,saveStation,loadStationDefaults,addDraftRule,removeDraft,clearDraftRules,draftPreset,renderCalvingGuide,startSession,setYard,finishSession,addCalendarEvent,renderCalendar,loadStrategyDefaults,calculateEconomics,saveScenario,calculateBullPower,saveBullPower,runEvidenceEngine,saveEvidenceRun,renderHistory,openSession,renderReport,exportCSV,downloadPDF,sharePDF,emailDetails,editRecord,backField,clearAll}
})();