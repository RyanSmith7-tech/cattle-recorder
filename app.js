const HerdIQ=(()=>{const KEY="HERDIQ_V52";
...
function save(){localStorage.setItem(KEY,JSON.stringify(S))}function load(){let r=localStorage.getItem(KEY)||localStorage.getItem("HERDIQ_V51")||localStorage.getItem("HERDIQ_V50");if(r)S=JSON.parse(r);S.stations??=[];S.sessions??=[];S.calendarEvents??=[];S.economicScenarios??=[];S.bullPowerScenarios??=[];S.evidenceRuns??=[];S.records??=[];S.draftRules??=[];S.draftRules.forEach(r=>r.wetDry??=[]);S.yard??={};S.animal??={}}
